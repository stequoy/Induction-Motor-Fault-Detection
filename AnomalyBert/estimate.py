...
def main(options):
    ...
    # Load test data.
    test_data = np.load('/Users/svirinstepan/PycharmProjects/AnomalyBert/dataset/processed/lasttry-2.npy').copy().astype(np.float32)
    ...
    # Estimate scores.
    if options.dataset == 'SEGMENTED':
        # Load the pickled list of numpy arrays
        with open('/Users/svirinstepan/PycharmProjects/AnomalyBert/dataset/processed/howto.pkl', 'rb') as file:
        ...
        for segment in test_data:
            ...
            output_values = estimate(segment, model, post_activation, n_column, options.batch_size,
                                     options.window_sliding, [[0, len(segment)]], options.check_count, device)
            predictions.append(output_values.cpu().numpy())

        # Save the predictions for all segments
        outfile = 'segmented_predictions.npy'
