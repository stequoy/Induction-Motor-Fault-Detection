...
def main(options):
    ...
    model = get_anomaly_transformer(input_d_data=d_data,
                                    ...
                                    dropout=options.dropout).to('cpu')
...
#     anomaly_weight = options.partial_loss / options.total_loss
    ...
    lr = options.lr
    if options.loss == 'l1':
        train_loss = nn.L1Loss().to('cpu')
        rec_loss = nn.MSELoss().to('cpu')
    elif options.loss == 'mse':
        train_loss = nn.MSELoss().to('cpu')
        rec_loss = nn.MSELoss().to('cpu')
    elif options.loss == 'bce':
        train_loss = nn.BCELoss().to('cpu')
#         train_loss = lambda pred, gt: -(gt * torch.log(pred + 1e-8) + (1 - gt.bool().float()) * torch.log(1 - pred + 1e-8)).mean()
        sigmoid = nn.Sigmoid().to('cpu')
    ...
    for i in range(options.initial_iter, max_iters):
        ...
        for j in first_index:
            x.append(torch.Tensor(train_data[j:j+data_seq_len].copy()).to('cpu'))
        x_true = torch.stack(x).to('cpu')
        ...
        for j, rep, tar, leng, typ, dim_num, dim_cat in zip(range(n_batch), replacing_index, target_index, replacing_lengths,
            ...
            if leng > 0:
                ...
                if typ > soft_replacing_prob:
                    ...
                    _x_temp = torch.stack(_x_temp).to('cpu')
                    weights = torch.from_numpy(replacing_weights(leng)).float().unsqueeze(0).to('cpu')
...
#                     _x[target_column_categorical] = torch.randint(0, 2, size=(rep_len_cat, 1), device=device).float()
                ...
                # Peak noising
                    ...
                    peak_value = (_x[target_column_numerical, peak_index] < 0.5).float().to('cpu')
...
#                     peak_value = (_x[target_column_categorical, peak_index] < 0.5).float().to('cpu')
                ...
                # Length adjusting (only for bce loss)
                    ...
                    # Shortening
                        ...
                        if origin_index > replacing_len_max * 1.5:
                            scale = np.random.randint(2, 5)
                            _leng = leng * (scale - 1)
                            x[j][:tar] = torch.Tensor(train_data[origin_index-_leng:origin_index+tar-_leng].copy()).to('cpu')
                            x[j][tar:tar+leng] = torch.Tensor(train_data[origin_index+tar-_leng:origin_index+tar+leng:scale].copy()).to('cpu')
                            x_anomaly[j, tar:tar+leng] = 1
...
#             loss += options.partial_loss * partial_loss
            ...
            for pred, gt, tar, leng in zip(y, x_rep, target_index, replacing_lengths):
                if leng > 0 and gt != None:
                    partial_loss += train_loss(pred[tar:tar+leng], gt.to('cpu'))
        ...
        if i % options.summary_steps == 0:
            with torch.no_grad():
                if options.loss == 'bce':
                    ...
                    #                      sigmoid if options.loss == 'bce' else nn.Identity().to('cpu'),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--replacing_data", default=None, type=str, help='external dabybyta for soft replacement; None(default)/SMAP/MSL/SMD/SWaT/WADI')
