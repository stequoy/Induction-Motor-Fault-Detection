...
def load_data(dataset, base_dir, output_folder, json_folder):
    ...
    if dataset == "BNLD":
        data_frame = pd.read_csv(os.path.join(base_dir, 'big_new_liman_dataset.csv'))
        numpy_array = data_frame.to_numpy()
        np.save(os.path.join(output_folder, dataset + "_train.npy"), numpy_array)


    if dataset == "LABELED":
        data_frame = pd.read_csv(os.path.join(base_dir, 'only_data_suffled_data_3bad_3good.csv'))
        data_frame = data_frame.head(150000)
        numpy_array = data_frame.to_numpy()
        np.save(os.path.join(output_folder, dataset + "_train.npy"), numpy_array)

    if dataset == "F3G":
        data_frame = pd.read_csv(os.path.join(base_dir, 'first_3_good_dataset.csv'))
        numpy_array = data_frame.to_numpy()
        np.save(os.path.join(output_folder, dataset + "_train.npy"), numpy_array)

    if dataset == "BAD_BNLD":
        data_frame = pd.read_csv(os.path.join(base_dir, 'big_new_liman_BAD_dataset.csv'))
        numpy_array = data_frame.to_numpy()
        np.save(os.path.join(output_folder, dataset + "_train.npy"), numpy_array)


    if dataset == 'LIMAN':
        data_frame = pd.read_csv(os.path.join(base_dir, 'LIMAN.csv'))
        # Extract only the columns for Currents A, B, and C
        ...
        np.save(os.path.join(output_folder, dataset + "_train.npy"), currents_array)

    if dataset == 'SIGN':
        # Path to your file
        file_path = os.path.join(base_dir, 'perecos_phasi.txt')

        # Read the file
        with open(file_path, 'r', encoding='latin1') as file:
            content = file.readlines()

        # Find the start index of the numeric data
        for index, line in enumerate(content):
            if line.strip() and line.split()[0].replace('.', '', 1).isdigit():
                data_start_index = index
                break

        # Extract the second column data
        second_column_data = []
        ...
        np.save(os.path.join(output_folder, "SIGN_train.npy"), reshaped_array)
...
if __name__ == '__main__':
    ...
    datasets = ['SMD', 'SMAP', 'MSL', 'SWaT', 'WADI', 'LIMAN', 'SIGN', 'BNLD', 'BAD_BNLD', 'F3G', 'LABELED']
